...
export default function About({
  ...
  // CV Preview Modal & Download States
  ...
  // Robust File Download (supports Sanity CDN ?dl= attachment and local files)
  const handleDownload = async (e?: React.MouseEvent) => {
    ...
    const filename = 'CV-Cris-Charles-Garcia.pdf';
    try {
      let targetUrl = resumeUrl;
      if (targetUrl.includes('cdn.sanity.io')) {
        const sep = targetUrl.includes('?') ? '&' : '?';
        targetUrl = `${targetUrl}${sep}dl=${encodeURIComponent(filename)}`;
      }
      ...
      if (fallbackUrl.includes('cdn.sanity.io')) {
        const sep = fallbackUrl.includes('?') ? '&' : '?';
        fallbackUrl = `${fallbackUrl}${sep}dl=${encodeURIComponent(filename)}`;
      }
      const link = document.createElement('a');
  ...
  return (
    <section id="about" className="py-12 sm:py-16 relative">
                      ...
                      <div key={idx} className={idx > 0 ? "pt-5 mt-5 border-t border-border-custom" : ""}>
                        ...
                        {edu.location && (
                          <div className="pt-2 text-[11px] font-mono text-muted-foreground-custom flex items-center gap-1.5">
                            <svg className="w-3.5 h-3.5 text-muted-foreground-custom shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      {/* Simple CV Preview Modal */}
      {mounted && isPreviewOpen && createPortal(
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-5 bg-black/70 backdrop-blur-sm transition-all"
          ...
          >
            ...
            <div className="px-4 sm:px-6 py-3.5 border-b border-border-custom flex items-center justify-between gap-3 bg-card-custom shrink-0">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-bold font-heading text-foreground-custom truncate">
                    Curriculum Vitae
                  </h3>
            ...
            <div className="relative flex-1 w-full h-full bg-zinc-900 overflow-hidden">
              <iframe
                ...
                title="Curriculum Vitae Document"
